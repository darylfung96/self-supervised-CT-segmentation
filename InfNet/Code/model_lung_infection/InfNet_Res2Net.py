...
from InfNet.Code.model_lung_infection.backbone.Res2Net import res2net50_v1b_26w_4s
...
class Inf_Net(nn.Module):
    ...
    def forward_inpainting(self, x):
        ...
        lateral_edge = self.edge_conv4_inpainting(edge_guidance)  # NOTES: Sup-2 (bs, 1, 88, 88) -> (bs, 1, 352, 352)
        lateral_inpainting_edge = F.interpolate(F.tanh(lateral_edge),
                                     scale_factor=4,
                                     mode='bilinear')
        ...
        ra2_feat = self.ra2_conv4_inpainting(x)
        x = F.tanh(ra2_feat + crop_2)
        lateral_inpainting_map_2 = F.interpolate(x,
                                      scale_factor=8,
                                      mode='bilinear')  # NOTES: Sup-4 (bs, 1, 44, 44) -> (bs, 1, 352, 352)

        return lateral_inpainting_map_5, lateral_inpainting_map_4, lateral_inpainting_map_3, lateral_inpainting_map_2, lateral_inpainting_edge
