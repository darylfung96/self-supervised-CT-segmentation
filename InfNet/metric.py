...
def sensitivity_similarity_coefficient(predicted_seg, ground_truth_seg):
    ...
    true_positive = (a * b).sum().detach().numpy()  # because ground truth is 1, and remove all the other false positive
    false_negative = (b - a).detach().numpy()
...
def specificity_similarity_coefficient(predicted_seg, ground_truth_seg):
    ...
    true_negative = (inverted_a * inverted_b).sum().detach().numpy()
    false_positive = (inverted_b - inverted_a).detach().numpy()
