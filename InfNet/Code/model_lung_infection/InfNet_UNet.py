...
class Inf_Net_UNet_Improved(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.up2 = Up(518, 256 // factor, bilinear) # 512 + 6 because of concatenation
        self.up3 = Up(256, 128 // factor, bilinear)
        self.up4 = Up(128, 64, bilinear)
        self.outc = OutConv(64, n_classes)
    ...
    def forward_inpainting(self, x):
        ...
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x3 = torch.cat((x3, F.interpolate(x, size=x3.shape[-1])), 1)
        x4 = self.down3(x3)
        x5 = self.down4(x4)
        x5 = torch.cat((x5, F.interpolate(x, size=x5.shape[-1])), dim=1)
